...
'loop_cs' => \$lloop_cs, 'loop_restrict|restrict' => \$lloop_restrict, 
...
if($lloop_diag || $lloop_max_ratio || $lloop_cs || $lloop_restrict){
	$lloop = 1;
}
if($lloop_restrict){
	$nclust_included = 'nclust_inc';
	if($lloop_max_ratio){
		die "\nOption --restrict is not for --loop_max_ratio.\n\n";
	}
}
...
if($lloop){
	...
	if($lloop_restrict && $nmol_types_used == 1){
		die "Found only one molecule type in the input file; option --restrict is for several molecule types\n";
	}elsif(($lloop_diag || $lloop_max_ratio) && $nmol_types_used != 2){
	...
	if(!$lloop_max_ratio){
		...
		for($imol=$nmol_types_used-1;$imol>=1;$imol--){
			...
			#$cluster_from_indices=$string3; # Fixed cluster numbers are not used anymore
		#}
	}else{
		...
		if($source_function_clust[$i] !~ /^n_j_in$/){
			...
			if(!$lloop){
				...
				if(!$lloop_max_ratio){
					...
					#$iclus = 0; # Fixed cluster numbers are not used anymore
...
if($lfortran){
	...
	# Do not print the monomer numbers anymore, as they are not fixed if the system is restricted (and probably it often is)
	#}else{
...
if($lfortran){
	...
	if($lloop && $nmol_types_used>1 && !$lloop_max_ratio){
	...
	if($lloop && !$lloop_diag){
		...
		if($nmol_types_used>1){
			...
			if($lloop_cs){
				...
				if(!$lloop_max_ratio){
					$lines{'l_evap'} .= "clust_from_indices($nmol_ranges_ij_ind).ne.0";
				}else{
			...
			if($l_jlim){
				...
				if(!$disable_evap){
					if($lloop_cs){
						...
						if(!$lloop_max_ratio){
							$lines{'l_evap'} =~ s/\.not\.all\(ij_ind\.eq\.ij_ind_cs\)/clust_from_indices\($nmol_ranges_ij_ind\)\.ne\.0/;
						}else{
							$lines{'l_evap'} =~ s/\.not\.all\(ij_ind\.eq\.ij_ind_cs\)/all\(ij_ind\.le\.ij_ind_max\)/;
						}
					}
					print OUT "\t\t\t\t\t$lines{'l_evap'}";
					if($nmol_types_used>1){
...
if($lloop){
	...
	if($nmol_types_used>1){
		...
		print OUT "\n";
		print OUT "\tinteger :: n, $nmol_ranges_ind, nclust_inc, indices(nclust,$nmol_types_used)\n";
		if($lloop_restrict){
			print OUT "\tlogical :: lvalid\n";
		}elsif($lloop_max_ratio){
			print OUT "\tinteger :: n_pure($nmol_ranges_array[$nmol_max_ratio_basis])\n\n";
			print OUT "\tcall pure_clust_indices$append_to_subroutine_names(n_pure)\n";
		}
		print OUT "\n";
		...
		if(!$lloop_max_ratio){
			...
			$string =~ s/\+/,/g;
			if($lloop_restrict){
				print OUT $str_temp."call check_validity((/$string/),lvalid)\n";
				print OUT $str_temp."if (lvalid) then\n";
				$str_temp2 = $str_temp."end if\n".$str_temp2;
				$str_temp .= "\t";
			}
			...
			if($lloop_cs){
				print OUT $str_temp."if (all(indices(n,:) .eq. ij_ind_cs)) then\n";
				$str_temp2 = $str_temp."end if\n".$str_temp2;
				$str_temp .= "\t";
		...
		if(!$lloop_max_ratio){
			...
			print OUT "\timplicit none\n";
			print OUT "\tinteger, parameter :: nclust = $max_cluster_number\n";
			print OUT "\tinteger :: i, $nmol_ranges_ind, nclust_inc, indices(nclust,$nmol_types_used), clust_from_indices($nmol_comp_list)\n\n";
			print OUT "\tclust_from_indices = 0\n";
			print OUT "\tcall get_molecule_numbers$append_to_subroutine_names(nclust_inc,indices)\n\n";
			print OUT "\tdo i = 1,nclust_inc\n";
			for($imol=1;$imol<=$nmol_types_used;$imol++){
				print OUT "\t\ti$imol = indices(i,$imol)\n";
			}
			$str_temp = $nmol_ranges_ind;
			$str_temp =~ s/,\s+/,/;
			print OUT "\t\tclust_from_indices($str_temp) = i\n";
			print OUT "\tend do\n";
		...
		if($loop_evap_coef =~ m/^DeltaG$/i){
			...
			if($disable_nonmonomer_evaps && $nmol_types_used==1){
				...
				if($nmol_types_used>1){
					...
					if(!$lloop_max_ratio){
						print OUT "clust_from_indices($nmol_ranges_ij_ind).ne.0";
					}else{
						print OUT "all(ij_ind.le.ij_ind_max)";
					}
					if($lloop_cs){
