...
data=flip((monY_coll_mat)./evap_tot_mat);

%%% FORMAT OF DATA
%if max(max(log10(data)))<2
%  pcolor_mat=[data NaN(size(data,1),1); NaN(1plot,size(data,2)+1)];
%else
