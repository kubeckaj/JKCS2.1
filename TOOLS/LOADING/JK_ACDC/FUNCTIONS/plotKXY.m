...
data=flip(monY_coll_mat+monX_coll_mat);
...
data_label=['({\it c}_{' Xname '}{\it\beta}_{' Xname '}+{\it c}_{' Yname '}{\it\beta}_{' Yname '}) [s^{-1}]'];
ylabel(cb,data_label,'LineWidth',Lwidth,'FontSize',Fsize,'FontWeight','demi')
ylabel(["\fontsize{20}" Yname])
xlabel(["\fontsize{20}" Xname])
title("\fontsize{20}Collision rate with "+Xname+" and "+Yname)
