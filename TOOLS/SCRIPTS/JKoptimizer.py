...
def cost_function(guesses,cost_function_file,QSCcpu,QSCparallel):
  ...
  for guess_i in range(len(guesses)):
    os.makedirs("XTB"+str(guess_i))
    os.chdir("XTB"+str(guess_i))
  ...
  if QSCparallel == 0:
    ...
    process = subprocess.Popen('sbatch --time 2:00:00 -n '+str(QSCparallel)+' --array=0-'+str(len(guesses)-1)+' JKsend "cd XTB\$SLURM_ARRAY_TASK_ID; sh '+cost_function_file+' '+str(QSCparallel)+'"', shell = True)
    process.wait()
 
  #WAITING/CHECKING JOBS TO BE FINISHED
  for guess_i in range(len(guesses)-1,-1,-1):
    while not os.path.exists('XTB'+str(guess_i)+'/result'):  
      time.sleep(5)
  time.sleep(1)
  ...
  results = []
  for guess_i in range(len(guesses)):
    with open('XTB'+str(guess_i)+'/result', 'r') as file:
      try:
        result = float(file.readline().strip())
      except:
        result = np.nan
    results.append(result)
    shutil.rmtree('XTB'+str(guess_i))
...
def generate_step(var, var_mod, var_mod_operation, idx):
  new_var = copy.deepcopy(var)
  new_var[idx] = operate(new_var[idx], var_mod, var_mod_operation)
...
def mover(initial_guess, var, ig_var_operation, var_mod, var_mod_operation, mod_mod, mod_mod_operation, og_cost, cost_function_file, cpu, parallel):
  ...
  #TODO ig_var_operation not used yet
  all_positions = [initial_guess*generate_step(var, where*var_mod[idx], var_mod_operation[idx], idx) for where in [1,-1] for idx in range(len(var))]
  ...
  for i in range(len(var)):
    ...
    if best_found > cost_backward[i]:
      ...
      best_var = copy.deepcopy(generate_step(var, -var_mod[i], var_mod_operation[i], i))
    ...
    if cost_forward[i] == og_cost or cost_backward[i] == og_cost or np.abs(den[i]) < 1e-99 or np.isnan(den[i]):
      if cost_forward[i] < og_cost:
        ...
        res.append(0)
        var_mod[i] = operate(var_mod[i],-mod_mod[i],mod_mod_operation[i])
        var_mod[i] = operate(var_mod[i],-mod_mod[i],mod_mod_operation[i])
      else:
        res.append(0)
      var_mod[i] = operate(var_mod[i],mod_mod[i],mod_mod_operation[i])
      ...
      #TODO var_mod_operation will change this for sure FUCKING TOUGH TO DO PROPERLY
      premove = ((cost_forward[i] - og_cost) * var_mod[i]) / den[i]
      ...
      res.append(premove)
    print("VAR "+str(i)+":",[cost_backward[i],og_cost,cost_forward[i],var[i],var_mod_operation[i],var_mod[i],mod_mod_operation[i],mod_mod[i],den[i],-res[i]],flush = True)
...
def moving(move, initial_guess, var, ig_var_operation, var_mod, var_mod_operation, mod_mod, mod_mod_operation, og_cost, cost_function_file, cpu, parallel):
  ...
  while testing != "END":
    ...
    if while_count*(in_parallel+1) > 50:
      ...
      testing = "END"
    #TODO: ig_var_operations will change this
    all_positions = [initial_guess*(var-np.sign(move)*array_operate(np.abs(move),-mod_mod,mod_mod_operation,friction_applied+i,1)) for i in range(in_parallel)]
    cost_all = cost_function(all_positions, cost_function_file, cpu, parallel)
    ...
    for test_i in range(len(cost_all)):
      ...
      if cost_all[test_i] < best_test_found:
        ...
        best_test_var =  copy.deepcopy(var-np.sign(move)*array_operate(np.abs(move),-mod_mod,mod_mod_operation,friction_applied+test_i,1))
      ...
      prev_var =  copy.deepcopy(var-array_operate(move,mod_mod,mod_mod_operation,friction_applied+test_i,1))
...
def main():
  ...
  parser.add_argument("-i", "--input", type=str, required=False, default="initial.txt", help="Input file w max 4 lines: initial guess, jac/hess step, increment, decrement")
  ...
  for iter in range(max_iter):
    ...
    if best_found < og_cost:
      ...
      #TODO this will be modified by ig_var_operations
      print('New Parameters:',var*initial_guess, flush = True)
      ...
      #TODO this will be modified by ig_var_operations
      print('Old Parameters:',var*initial_guess, flush = True)
      print('Cost function: ',og_cost, " kcal/mol/atom (original was ",rem_og_cost,"kcal/mol/atom)", flush = True)
