...
def numeric_gradient(parr,initial0,diff_step,og_cost):
    if Qparallel > 1:
      cost_all = Parallel(n_jobs=num_cores)(delayed(parr_cost_function)(i) for i in [initial0*generate_step(parr,idx,diff_step) for idx in range(len(parr))])
...
def lets_optimize(parr,initial0,diff_step,og_cost,hess_step):
    ...
    for iter in range(max_iter):
        ...
        process = subprocess.Popen('NUMBA=$(grep "AAA" param_gfn1-xtb.txt -o | wc -l);TEST=($(cat LAST_parameter.txt));for ((i = 1; i <= NUMBA; i++)); do loop_counter=$((i - 1)); cp param_gfn1-xtb.txt LAST_param_gfn1-xtb.txt; sed -i "s/AAA$i /${TEST[$loop_counter]} /g" LAST_param_gfn1-xtb.txt;done', shell = True)
        process.wait()
