...
train_high_database, train_low_database, test_high_database, test_low_database, monomers_high_database, monomers_low_database = load_databases(Qtrain, Qeval, Qopt, Qmonomers, method, VARS_PKL, TRAIN_HIGH, TRAIN_LOW, TEST_HIGH, TEST_LOW, MONOMERS_HIGH, MONOMERS_LOW)
...
if Qsampleeach > 0:
  ...
  sampleeach_all, sample_train, sample_test = sampleeach_mbtr(train_high_database, test_high_database) 
elif Qsampleeach < 0:
  from src.sampleeach import sampleeach_fchl
  sampleeach_all, sample_train, sample_test = sampleeach_fchl(train_high_database, test_high_database, krr_cutoff) 
else:
  sampleeach_all = ["once"]
...
####################################################################################################
  ...
  if Qtrain == 1:
    ...
    locals().update(prepare_data(train_high_database, monomers_high_database, train_low_database, monomers_low_database, seed, size, method, column_name_1, column_name_2, Qmin, Qifforces, Qifcharges, Qifdipole, Qsampleeach, Qforcemonomers, sampledist, Qmonomers))
    ...
    ###################################
      ...
      training(Qforces,Y_train,F_train,D_dipole,Q_charge,Q_charges,strs,nn_atom_basis,nn_rbf,nn_interactions,nn_cutoff,nn_tvv,Qbatch_size,seed,nn_epochs,Qlearningrate,varsoutfile)
    ###################################
    else:
      print("JKML: Wrong method ("+Qmethod+") or representation ("+Qrepresentation+") chosen.")
      exit()
    ###################################
...
####################################################################################################
  ...
  if Qopt > 0:
    ...
    if Qmethod == "krr" and Qrepresentation == "fchl":
      ...
      optimize(strs,Qrepresentation,krr_cutoff,varsoutfile,Qsplit,nn_cutoff,Qkernel,Qopt,opt_maxstep,opt_dump,opt_steps,md_temperature,md_thermostatfriction,md_dump,md_steps)
    #####################################
    elif Qmethod == "nn":
      from src.SchNetPack import optimize
      optimize(test_high_database,varsoutfile,nn_cutoff,Qopt,opt_maxstep,opt_dump,opt_steps,md_temperature,Qmd_timestep,md_thermostatfriction,md_dump,md_steps)
    #####################################
    elif Qmethod == "physnet":
...
####################################################################################################
####################################################################################################
print("JKML has finished", flush = True)
