def substract_monomers(
    the_clusters_df, the_monomers_df, Qmonomers, column_name_1, column_name_2
):
    ...
    if Qmonomers == 2:  # no difference from monomers at all
        ...
        if Qmonomers == 1:
            ...
            ins_monomers = [
                sum(clusters_df0["info"]["component_ratio"].values[i]) == 1
                for i in range(len(clusters_df0))
            ]
            monomers_df = clusters_df0[ins_monomers]
        else:
            ins_monomers = [
                sum(the_clusters_df["info"]["component_ratio"].values[i]) == 1
                for i in range(len(the_clusters_df))
            ]
        ...
        print(
            "JKML(data): FOR JAKUB: I should check whether these are unique:"
            + str(monomers_df["info"]["components"].values)
            + "\n"
        )
        for i in transpose(
            [
                the_clusters_df["info"]["components"].values,
                the_clusters_df["info"]["component_ratio"].values,
            ]
        ):
            nn = 0
            # print(i)
            ...
            for j in all_mons:
                ...
                for k in range(len(monomers_df)):
                    ...
                    if j == monomer_k_name:
                        the_ens_correction[n] += (
                            monomer_k[column_name_1][column_name_2]
                            * all_mons_ratios[nn]
                        )
                        test = 1
                        break
...
def prepare_data_for_training(
    ...
    if method == "delta":
        ...
        if ens.shape != ens2.shape:
            print(
                "JKML(data): The LOW and HIGH method train sizes do not match. [EXITING]"
            )
            exit()
    if method == "min":
        ens -= Qmin
    ...
    ### CHARGE
    if (
        ("log", "charge") in clusters_df.columns
        and ("log", "mulliken_charges") in clusters_df.columns
        and Qifcharges == 1
    ):
    ...
    # HIGH LEVEL
    ens_correction = substract_monomers(
        clusters_df, monomers_high_database, Qmonomers, column_name_1, column_name_2
    )
    # LOW LEVEL
    if method == "delta":
        ens2_correction = substract_monomers(
            clusters_df2, monomers_low_database, Qmonomers, column_name_1, column_name_2
        )
