...
def training(Qrepresentation,Qkernel,Qsplit,strs,Y_train,krr_cutoff,lambdas,sigmas,varsoutfile,Qsplit_i,Qsplit_j,kernel_args):
  ...
  if Qrepresentation == "fchl":
    ...
    for i in range(len(repres_dataframe)):
      ...
      repres_dataframe["xyz"].iloc[i] = generate_representation(strs.iloc[i].get_atomic_numbers(), strs.iloc[i].get_positions(), max_size = max_atoms, neighbors = max_atoms, cut_distance=krr_cutoff)
    X_atoms = None
    X_train = np.array([mol for mol in repres_dataframe["xyz"]])
  elif Qrepresentation == "mbdf":
    X_atoms = [strs[i].get_atomic_numbers() for i in range(len(strs))]
    X_train = generate_representation(np.array([i.get_atomic_numbers() for i in strs]), np.array([i.get_positions() for i in strs]), cutoff_r = krr_cutoff, normalized = False)

  #some info about the full representation
  print("JKML(QML): Shape of the training representation: "+str(X_train.shape), flush = True)
  ...
  return {key: value for key, value in locals().items() if key == 'X_train' or key == 'X_atoms' or key == 'alpha'}
...
def evaluate(Qrepresentation,krr_cutoff,X_train,sigmas,alpha,strs,kernel_args,Qkernel):
  ...
  if Qrepresentation == "fchl":
    ...
    X_test = generate_representation(np.array([i.get_atomic_numbers() for i in strs]), np.array([i.get_positions() for i in strs]), cutoff_r = krr_cutoff, normalized = False)

  #some info about the full representation
  print("JKML(QML): Shape of the testing representation: "+str(X_test.shape), flush = True)
...
def optimize(strs,Qrepresentation,krr_cutoff,varsoutfile,Qsplit,nn_cutoff,Qkernel,Qopt,opt_maxstep,opt_dump,opt_steps,md_temperature,md_thermostatfriction,md_dump,md_steps):
  ...
  print("JKML(QML): WARNING: These are just some shit numerical calculations", flush = True)
  ...
  for step in range(maxsteps):
    ...
    print("JKML(QML): "+str(step)+" \t"+str(save_energy)+" \t"+str(maxdev),flush = True)
