...
'''
This is a modified ArbAlign script meant to be used with JKTS
'''
...
def transform_coords(coords, swap, reflect):
   ...
   for i in range(len(coords)):
      new_coords.append([coords[i][swap[0]]*reflect[0], coords[i][swap[1]]*reflect[1], coords[i][swap[2]]*reflect[2]])
   return new_coords
...
def calculate_rmsd(molecule1, molecule2, noHydrogens=False):
   rmsd_value = 0.0
   # Read in original coordinates and labels of xyz1 and xyz2
   a_labels, a_coords, NA_a = read_xyz_from_molecule(molecule1, noHydrogens)
   b_labels, b_coords, NA_b = read_xyz_from_molecule(molecule2, noHydrogens)
   ...
   if InitRMSD_unsorted < 0.001:
      ...
      # print("All-atom RMSD: %e" % float(InitRMSD_unsorted))
      return InitRMSD_unsorted
   ...
   """
   a_labels, a_coords, NA_a, order = sorted_xyz_from_molecule(molecule1)
   Uniq_a = list(set(a_labels))
   list.sort(Uniq_a)
   N_uniq_a = len(Uniq_a)
   Atom_freq_a = dict(Counter(a_labels))

   b_labels, b_coords, NA_b, junk = sorted_xyz_from_molecule(molecule2)
   ...
   for i in range(len(B_t)):
      l = 0
      cost_matrix = np.array([[np.linalg.norm(a - b) for b in B_t[i][0]] for a in A])
      LAP = linear_sum_assignment(cost_matrix)
      vars()[Perm[Uniq[l]]] = []
      for j in range(len(LAP[1])):
         vars()[Perm[Uniq[l]]] += [(j,LAP[1][j])]
      ...
      if num_uniq == 1:
         ...
         while l < num_uniq:
            ...
            a_coords_array = np.array(vars()[a_Coords[Uniq[l]]])
            b_coords_array = np.array(vars()[b_Coords[Uniq[l]]])
            cost_matrix = np.array([[np.linalg.norm(a-b) for b in b_coords_array] for a in a_coords_array])
            LAP = linear_sum_assignment(cost_matrix)
            vars()[Perm[Uniq[l]]] = []
            for k in range(len(LAP[1])):
               vars()[Perm[Uniq[l]]] += [(k,LAP[1][k])]
            vars()[Perm[Uniq[l]]] = sorted( vars()[Perm[Uniq[l]]], key = lambda x: x[0])
            vars()[Perm[Uniq[l]]] = [x[1] for x in vars()[Perm[Uniq[l]]]]
...
def main(molecules, RMSD_threshold):
    unique_molecules = []
